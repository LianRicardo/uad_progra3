...
public:
	...
	void LoadAnimCurves(Take* dst, const char* mdlName, const char* channelName)
	{
	...
	void LoadModelProperties(Model* mdl)
	{
		...
		mdl->localRotate *= (2.f / 360.f);

		mdl->CalcLocalMatrixFromSRT();
		fbx_printf(mdl->localMatrix);
	}

	void LoadModel(Model* mdl, const char modelName[], const char modelType[])
	{
...
void LoadFbx(FbxScene* pScene, FbxUtil::Stream& file)
{
